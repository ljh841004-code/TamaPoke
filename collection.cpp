...
void Collection::begin() {
  ...
  if (version == 3) {
    ...
    static StoredMonV2 old[151];
    if (prefs.getBytes("mons", old, sizeof(old)) == sizeof(old)) {
      for (int i = 0; i < 151; ++i) {
        // Existing fields retain their offsets; only the final name grows.
        memcpy(&mons[i], &old[i], offsetof(StoredMonV2, nick));
        memcpy(mons[i].nick, old[i].nick, sizeof(old[i].nick));
        mons[i].nick[sizeof(old[i].nick) - 1] = 0;
      }
      save();
    }
  } else if (version == 1) {
    ...
    if (prefs.getBytes("mons", old, sizeof(old)) == sizeof(old)) {
      for (int i = 0; i < 151; i++) {
        ...
        memcpy(d.nick, s.nick, sizeof(s.nick));
