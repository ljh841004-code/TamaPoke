...
## Personal update v1.19

Based on the supplied v1.18 personal patch at upstream commit 67aa88932ed81cfe12e391597b38c694fd52bcd6.
All personal audio levels, cries, training, Hangul names, HP displays and save migrations are retained.
Music streams PCM 16 kHz / 16-bit / mono WAV from SD with an 8 KiB read-ahead buffer.
`/mons/bgm.wav` loops to its full data-chunk end. Active wild battles use
`/mons/battle_wild.wav`. Results, escape, trainer waves and screen closure return
to ordinary music from the beginning. The next wild wave in the same run resumes
the battle song; a new run resets it. Muting/sleep pauses playback.

Only the separately supplied `bgm.wav` and `battle_wild.wav` need copying to the
existing SD `/mons` directory. Existing `cry001.wav` through `cry151.wav` remain.
No user music is included in the Git source. The legacy `tools/build_audio.py`
generates the old original sample BGM; do not use it to replace the supplied songs.

Use the separate v1.19 Windows installer for save-preserving app-only updates.
Its default operation backs up all flash, checks the partition table, flashes
only the app at 0x10000 and confirms the reported version after reboot.
`--with-audio` optionally transfers the separate BGM folder; `--audio-only` only
transfers it after confirming v1.19. See `installer/README_KO.txt`.
The merged image / web erase installer is for a fresh installation, not an update
that must preserve saves.
