...
야생 배틀, 통신 대전/교환(ESP-NOW), WiFi 시간 자동 맞춤, 한국어 기본, SD카드 소리(배경음·배틀 배경음·울음소리)를 더했어요.

## 폴더 구성

| 파일 | 내용 |
|---|---|
| `tamapoke-ko-v1.17-ko2-app-0x10000.bin` | **업데이트용** (ko1 → ko2). 주소 **0x10000**. 키우던 포켓몬이 그대로 남아요 |
| `tamapoke-ko-v1.17-ko2.bin` | **새로 설치용** 통합 이미지. 주소 **0x0**. 저장 데이터가 초기화돼요 |
| `TamaPoke_v1.17-ko2_정리.txt` | 기능·사용법 요약 메모 |
...
2. Connect → 보드 포트 선택
3. 아래 중 하나 → Program
   - ko1을 쓰고 있다면: 주소 `0x10000`에 `tamapoke-ko-v1.17-ko2-app-0x10000.bin` (Erase Flash 누르지 않기)
   - 처음 설치하거나 초기화하고 싶다면: 주소 `0x0`에 `tamapoke-ko-v1.17-ko2.bin`
4. "Hard resetting via RTS pin..."이 뜨면 완료. 화면이 안 바뀌면 RESET 버튼 또는 USB 다시 연결
5. 시계 화면(아래로 스와이프) 아래쪽에 `v1.17-ko2`가 보이면 성공
...
SD카드 `/mons/` 폴더에 `bgm.wav`(평소), `battle_wild.wav`(배틀 중), `cry001.wav`~`cry151.wav`(울음소리)를 넣고 재부팅.
